...
def BoutonOn(x:int, y:int, num_tour:int):
    """Surligne le bouton appuyé"""
    
    souris.goto(x,y)
    souris.down()
    souris.fillcolor("darkgrey")
    souris.begin_fill()
    rectangle(110,30)
    souris.end_fill()
    souris.up()
    souris.goto(souris.xcor()+55, souris.ycor()-24)
    souris.write("Tour "+str(num_tour),align="center", font=("Arial",10, "normal"))
    
def BoutonOff(x:int, y:int, num_tour:int):
    """Remet le bouton surligné à son état normal"""
    
    souris.goto(x,y)
    souris.down()
    souris.fillcolor("lightgrey")
    souris.begin_fill()
    rectangle(110,30)
    souris.end_fill()
    souris.up()
...
def lireCoords(plateau: list, dep:int, arr:int) -> tuple[int, int]:
    ...
    if dep == 0 or arr == 0:
        BoutonOff(-(screensizeX/2)+120,(screensizeY/2)-200,0)
    if dep == 1 or arr == 1:
        BoutonOff(-(screensizeX/2)+120,(screensizeY/2)-250,1)
    if dep == 2 or arr == 2:
        BoutonOff(-(screensizeX/2)+120,(screensizeY/2)-300,2)
...
def jouerClic(x:int,y:int):
    ...
    if -(screensizeX/2)+120 <= x <= (screensizeX/2)+230:
        if (screensizeY/2)-300 >= y >= (screensizeY/2)-330: #Tour 2
            clics.append(2)
            BoutonOn(-(screensizeX/2)+120,(screensizeY/2)-300,2)
        elif (screensizeY/2)-250 >= y >= (screensizeY/2)-280: #Tour 1
            clics.append(1)
            BoutonOn(-(screensizeX/2)+120,(screensizeY/2)-250,1)
        elif (screensizeY/2)-200 >= y >= (screensizeY/2)-230: #Tour 0
            clics.append(0)
            BoutonOn(-(screensizeX/2)+120,(screensizeY/2)-200,0)
